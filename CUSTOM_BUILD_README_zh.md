...
4. 通用多层 KeyMap（PC 客户端）
   ...
   - 对于背包、地图等界面，可在普通点击节点上设置 `"switchMap": true`。例如 `Key_Tab`：按下和松开仍会完成其游戏内点击，松开后在 FPS 触摸映射和 Android UHID 平板虚拟鼠标之间切换；它不会合成一个 `~` 键事件，也不切换 KeyMap Layer。
   ...
   - 离线可视化编辑器位于 `tools/layer-keymap-editor/`：直接打开 `index.html`，或运行 `node server.js` 后访问本机预览。它支持旧单层和新的 Layer JSON、点位拖动、Layer 切换字段编辑、方向轮盘的 WASD 四向偏移量编辑、配置校验和本地 JSON 导出；游戏截图可作为本地背景图核对点位，不会写入导出的 JSON。

...
普通“点击”节点还可勾选“松开后释放 / 重新捕获鼠标”。这就是旧配置的 `switchMap: true`：该按键仍会发送到游戏，松开后切换 FPS 鼠标捕获，效果上相当于手动释放或重新捕获鼠标，适合 `Tab` 背包或 `M` 地图。它不会合成一个 `~` 键事件，也和 `switchLayer` 相互独立，不能把它当成图层切换使用。

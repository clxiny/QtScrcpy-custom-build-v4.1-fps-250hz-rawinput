...
$PatchFiles = @(
    (Join-Path $PatchDir "0001-add-server-side-video-pause.patch"),
    (Join-Path $PatchDir "0002-add-input-state-recovery.patch")
)
...
foreach ($PatchFile in $PatchFiles) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to apply server patch: $PatchFile"
        }
    } else {
        git -C $SourceDir apply --reverse --check $PatchFile 2>$null
        if ($LASTEXITCODE -ne 0) {
            throw "The scrcpy source tree is neither clean nor already patched for: $PatchFile"
        }
    }
}
